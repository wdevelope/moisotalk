...
export async function POST(req: NextRequest) {
  ...
  if (points <= 0) return NextResponse.json({ error: 'insufficient_points', points }, { status: 403 });
  ...
  return NextResponse.json({ message: inserted, points: newPoints, deducted: hasKorean });
}
